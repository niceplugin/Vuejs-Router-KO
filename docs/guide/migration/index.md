...
### Removed `*` (star or catch all) routes %{#removed-star-or-catch-all-routes}%
...
### `<router-view>`, `<keep-alive>`, and `<transition>` %{#router-view-keep-alive-and-transition}%
...
### Removal of `pathToRegexpOptions` %{#removal-of-pathtoregexpoptions}%
...
### `routes` option is required in `options` %{#routes-option-is-required-in-options}%
...
### Named children routes with an empty `path` no longer appends a slash %{#named-children-routes-with-an-empty-path-no-longer-appends-a-slash}%
